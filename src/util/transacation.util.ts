...
export class TransactionUtil {
    ...
    public async restPostTransaction<T = any>(querystring: string, param: T, headerOpts: T): Promise<any> {
        try {
    ...
    public async restGetTransaction<T = any>(querystring: string, headerOpts: T): Promise<any> {
        try {
            let header = this.setHeaderData(headerOpts);
            let hres: restClient.IRestResponse<T>;
    ...
    private setHeaderData<T = any>(header: T): IRequestHeaderOptions {
        ...
        restOptions.acceptHeader = 'application/json';
